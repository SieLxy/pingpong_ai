...
乒乓球动作识别系统。公开页可上传视频进行识别；管理员登录后可标注新视频并训练模型。
所有运行日志写入 `logs/run-YYYYMMDD-HHMMSS.log`（或 `LOG_FILE_PATH` 指定的文件）。
...
## 运行环境
- Windows 10/11
- Python 3.11（start.bat 优先使用 C:\Users\Administrator\AppData\Local\Programs\Python\Python311\python.exe）
- 依赖：FastAPI, Uvicorn, MediaPipe, OpenCV, scikit-learn 等（见 requirements.txt）
...
```
流程：
1) 生成时间戳日志文件 `logs/run-YYYYMMDD-HHMMSS.log`，设置环境变量 `LOG_FILE_PATH` 供应用使用。
2) 创建/复用 `.venv`，安装依赖。
3) 启动 Uvicorn（端口 8000），日志写入日志文件。

## 使用步骤
- 识别：浏览器打开 http://127.0.0.1:8000 ，上传视频即可。
- 管理员训练：
...
- `LOG_FILE_PATH`：自定义日志文件路径；若未设置，应用写 `logs/app.log`，但 start.bat 会同时生成 run-*.log 并通过该变量覆盖。
